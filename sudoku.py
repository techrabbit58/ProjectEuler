"""Support functionality to solve Sudoku puzzles via backtracking.

B.t.w., in the test routine, this solves also Project Euler problem #96,
'Su Doku', which wants to retrieve a checksum from the integers formed by the
top left three fields of 50 solved Sudoku puzzles.

The puzzles must be read from the file 'p096_sudoku.txt'.
"""

from helpers import chronometric


def parse_project_euler_format(fp):
    """Project Euler uses the following format to store Sudoku puzzles:
...
@chronometric
def main():
    with open('p096_sudoku.txt') as fp:
        sudoku_collection = parse_project_euler_format(fp)
        checksum = 0
        for sudoku in sudoku_collection:
            solved_puzzle = solve(sudoku.puzzle)
            print(sudoku.name, solved_puzzle)
            checksum += int(solved_puzzle[:3])
    return checksum


if __name__ == '__main__':
    solution, elapsed = main()
    print('solution =', solution)
    print('runtime =', elapsed, 's')

__all__ = ['parse_project_euler_format', 'Sudoku', 'solve']
